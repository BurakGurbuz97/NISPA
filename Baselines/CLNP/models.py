...
class ConvNet(nn.Module):
    def __init__(self, input_channels, output_dim, conv2lin_kernel_size = 64):
        ...
        self.fc1 = MaskedLinearDynamic(128 * self.conv2lin_kernel_size, 1024)  # 5x5 image dimension
    ...
    def re_initialize(self, freeze_masks):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                old_weights = m.weight.data.clone().detach()
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                m.weight.data[torch.tensor(freeze_masks[i][0])] = old_weights[torch.tensor(freeze_masks[i][0])]
                
                old_bias = m.bias.clone().detach()
                nn.init.constant_(m.bias.data, 0)
                m.bias.data[torch.tensor(freeze_masks[i][1])] =  old_bias[torch.tensor(freeze_masks[i][1])]
                i += 1
...
class MaskedLinearDynamic(nn.Linear):
    def __init__(self, in_features, out_features, bias=True):
        ...
        #Initially we have dense network
        self.weight_mask = torch.ones_like(self.weight.data)
        self.bias_mask = torch.ones_like(self.bias.data)

...
def SplitGen(train_dataset, val_dataset, test_dataset, first_split_sz=2, other_split_sz=2, rand_split=False, remap_class=False):
    '''
    ...
    '''
    assert train_dataset.number_classes==val_dataset.number_classes and test_dataset.number_classes==val_dataset.number_classes,'Train/Val has different number of classes'
    ...
    while split_boundaries[-1]<num_classes:
        if len(other_split_sz) == 1:
            split_boundaries.append(split_boundaries[-1]+other_split_sz[0])
        else:
            split_boundaries.append(split_boundaries[-1]+other_split_sz[task_id])
            task_id = task_id + 1
    ...
    for name,class_list in class_lists.items():
        ...
        test_dataset_splits[name] = AppendName(Subclass(test_dataset, class_list, remap_class), name)

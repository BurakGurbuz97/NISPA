...
def export_results(filename, setting):
...
def run(args):
    ...
    # Prepare dataloaders
    train_dataset, val_dataset, test_dataset = dataloaders.base.__dict__[args.dataset](args.dataroot, args.train_aug, args.seed)

    train_dataset_splits, val_dataset_splits, test_dataset_splits, task_output_space = SplitGen(train_dataset, val_dataset, test_dataset,
    ...
    agent_config = {'lr': args.lr, 'momentum': args.momentum, 'weight_decay': args.weight_decay,'schedule': args.schedule,
                    ...
                    'reg_coef':args.reg_coef, 'n_samples': args.n_samples,
                    "e_stopping_wait": args.e_stopping_wait}
    ...
    if args.offline_training:  # Non-incremental learning / offline_training / measure the upper-bound performance
        ...
        for i in range(len(task_names)):
            ...
            for j in range(i+1):
                ...
                print('Test split name:', test_name)
                test_data = test_dataset_splits[test_name]
                test_loader = torch.utils.data.DataLoader(test_data,
                                                         batch_size=args.batch_size, shuffle=False,
                                                         num_workers=args.workers)
                acc_table[test_name][train_name] = agent.validation(test_loader)
...
def get_args(argv):
    ...
    parser.add_argument('--model_name', type=str, default='MLPCustom', help="The name of actual model for the backbone")
    ...
    #TASK5 Stands for EFMNIST task seqence
    parser.add_argument('--dataset', type=str, default='TASK5', help="|CIFAR10|CIFAR100|TASK5")
    ...
    parser.add_argument('--other_split_size', type=int, nargs='+', default=[13,13,11,10])
    ...
    parser.add_argument('--schedule', type=int,  nargs="+", default=[2],
    ...
    parser.add_argument('--repeat', type=int, default=1, help="(Not Available!!!) Repeat the experiment N times")
    ...
    parser.add_argument('--n_samples', type=int, default=None, help="Number of Samples for EWC fisher matrix")
    parser.add_argument('--e_stopping_wait', type=int, default=3)
...
if __name__ == '__main__':
    ...
    for reg_coef in reg_coef_list:
        ...
        for r in range(args.repeat):
            ...
            print(acc_table)
            export_results(args.experiment_name, setting)
